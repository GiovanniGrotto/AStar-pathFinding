...
const Pathfind=()=>{
  ...
  async function visualizePath(){ //funzione attivata quando si preme il bottone
    setStartEnd();          //funzione che passa tutta la griglia e ricnosce inizio e fine
    setWall();
    ...
    for(let i=0;i<3;i++){
      clearGrid()
      let path=Astar(startNode,endNode,i);  //l'algoritmo Astar restituisce il percorso migliore e i nodi visitati
  ...
  function animation(mode){
    for(let i=0;i<=visitedNodes.length;i++){   //scorriamo la lista dei nodi visitati
      if(i === visitedNodes.length){ //all'ultimo nodo chiamiamo la funzione per disegnare il percorso
        setTimeout(()=>{
          visualizeShortest(finalPath);
        },RENDER_RATE*2*i);
      }else{    //se il nodo non è l'ultimo viene disegnato come visitato
        setTimeout(()=>{
    ...
    if(mode===0){
      ...
      let node = document.createElement('li');
      node.appendChild(document.createTextNode("NodeVisited with tutorialHeuristic :"+visitedNodes.length));
      document.querySelector('ul').appendChild(node);
  ...
  function setStartEnd(){
    for(let i=0;i<rows;i++){
      for(let j=0;j<cols;j++){
        ...
        if(!node.isStart && !node.isEnd){ // se non è nessuno dei due mi assicuro che diventi un nodo normale, a volte erano non colorati ma in grid ancora start e end
